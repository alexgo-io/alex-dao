...
const agp003Address = "ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE.agp003-launchpad-alex";
const agp005Address = "ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE.agp005-wstx-alex-50-50";
const agp006Address = "ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE.agp006-flash-loan-fee";
...
class DAO {
    ...
    transferToken(sender: Account, token: string, amount: number, receiver: string, memo:ArrayBuffer) {
      let block = this.chain.mineBlock([
          Tx.contractCall(token, "transfer-fixed", [
            types.uint(amount),
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
      ...
      result = await DAOTest.transferToken(deployer, 'token-wstx', 1_000_000e8, daoAddress, new ArrayBuffer(4));
      result.expectOk();

      result = await DAOTest.executiveAction(deployer, agp005Address);
      result.expectOk();

      let call = chain.callReadOnlyFn("fixed-weight-pool", "get-pool-contracts",[ types.uint(1) ], deployer.address);
      result = call.result.expectOk().expectTuple();
      result['token-x'].expectPrincipal(deployer.address + '.token-wstx');
      result['token-y'].expectPrincipal(deployer.address + '.age000-governance-token');      
      result['weight-x'].expectUint(0.5e8);
      result['weight-y'].expectUint(0.5e8);

      call = chain.callReadOnlyFn("fixed-weight-pool", "get-pool-details", 
      [ 
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
      ...
      let call = chain.callReadOnlyFn("alex-vault", "get-flash-loan-fee-rate", [], deployer.address);
